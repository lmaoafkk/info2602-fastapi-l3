...
def todos_output():
    with get_session() as db:
        ...
        for todo in todos:
            print(f"ID: {todo.id} | User: {todo.user.username} | Done: {todo.done} | Text: {todo.text}")
...
@cli.command()
def mark_all_user_complete(username:str):
    with get_session() as db:
        user = db.exec(select(User).where(User.username == username)).one_or_none()
        if not user:
        ...
        db.commit()
        print(f"Marked {len(incomplete_todos)} todos complete for {username}")
